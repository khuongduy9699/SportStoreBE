...
const LoginWithUsers = async (req, res) => {
    ...
    return res
        .status(200)
        .json({ message: 'Error phone number! Check your phone number, Please!' });
};
            ...
            jwt.verify(refreshToken, process.env.JWT_REFRESH_SECRET, (err, data) => {
                ...
                if (err) {
                    ...
                    const accessToken = jwt.sign(
                        { data: { id: data.id, Name: data.Name, Phone: data.Phone } },
                        process.env.JWT_SECRET,
                        { expiresIn: '60s' },
                    );
...
const LoginWithAdmin = async (req, res) => {
    ...
    if (user) {
        ...
        if (check) {
            ...
            return res.status(200).json({
                message:
                    'Error phone number or password! Check your phone number and password, Please!',
            });
        } else {
            return res.status(200).json({
                message:
